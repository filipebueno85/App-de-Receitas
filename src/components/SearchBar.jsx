...
function SearchBar() {
  ...
  const handleClick = () => {
    if (searchEndpoint === 'ingredient') {
      if (options === 'meals') {
        return mealsApi(`filter.php?i=${searchValue}`);
      }
      return drinksApi(`filter.php?i=${searchValue}`);
    }
    if (searchEndpoint === 'name') {
      if (options === 'meals') {
        return mealsApi(`search.php?s=${searchValue}`);
      }
      return drinksApi(`search.php?s=${searchValue}`);
    }
    if (searchEndpoint === 'first-letter') {
      ...
      return drinksApi(`search.php?f=${searchValue}`);
    }
  };
